...
let handler = async (m, { usedPrefix, command, text }) => {
  ...
  if (!text) {
    return m.reply(global.t('zodiaco_usage', userId, groupId, {
      example: `${usedPrefix + command} 2003 02 25`
    }));
  }
  ...
  const verificaEta = mese === nascita[1] && giorno === nascita[2]
    ? global.t('zodiaco_birthday', userId, groupId, { eta })
    : global.t('zodiaco_age', userId, groupId, { eta });

  const descrizioneSegno = getDescrizioneSegno(segnoZodiacale);
  const emojiSegno = getZodiacEmoji(segnoZodiacale);
  const oroscopo = getRandomHoroscope(segnoZodiacale);

  const textbot = '> ChatUnity Bot';

  const testo = global.t('zodiaco_profile', userId, groupId, {
    nascita: nascita.join('-'),
    compleanno: prossimoCompleanno.join('-'),
    eta: verificaEta,
    segno: segnoZodiacale,
    emoji: emojiSegno,
    descrizione: descrizioneSegno,
    oroscopo,
    bot: textbot
  });
...
handler.command = /^(zodiaco|segno|oroscopo|zodiac|sign|horoscope)$/i;
