...
async function before(m, { conn }) {
    // Verifica che sia chat privata e che sia owner
    if (m.isGroup) return;
    if (!m.text) return;
    ...
    console.log('🔗 Link WhatsApp rilevato in privato:', m.text);
    console.log('📱 Da:', m.sender);
    
    // Verifica che sia owner
    const isOwner = [conn.decodeJid(global.conn.user.id), ...(Array.isArray(global.owner) ? global.owner.map(([number]) => number) : [])].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender);
    
    if (!isOwner) {
        console.log('❌ Utente non autorizzato, ignoro il link');
        return;
    }
    
    console.log('✅ Owner verificato, procedo con l\'analisi del gruppo');
    
    // Estrai il codice di invito dal link
    let [_, code] = m.text.match(linkRegex) || [];
    if (!code) {
        console.log('❌ Codice invito non valido');
        return m.reply('『 ❌ 』 *Link non valido*\n\n' +
                      '✅ Formato corretto: https://chat.whatsapp.com/xxxxxxxxx');
    }
    
    console.log('📋 Codice invito estratto:', code);
    ...
    try {
        ...
        let groupInfo;
        try {
            console.log('🔍 Ottengo informazioni sul gruppo...');
            groupInfo = await conn.groupGetInviteInfo(code);
            console.log('✅ Info gruppo ottenute:', groupInfo.subject, '- Membri:', groupInfo.size);
        } catch (error) {
            console.log('❌ Errore ottenimento info gruppo:', error.message);
        ...
        if (groupInfo.size < MIN_MEMBERS) {
            console.log(`❌ Gruppo troppo piccolo: ${groupInfo.size}/${MIN_MEMBERS}`);
        ...
        try {
            ...
            console.log('✅ Bot non ancora nel gruppo, procedo');
        }
        ...
        console.log('🚀 Tentativo di ingresso nel gruppo...');
        ...
        console.log(`✅ Bot entrato con successo nel gruppo: ${joinResult}`);
