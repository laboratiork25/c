...
export async function participantsUpdate({ id, participants, action }) {
    ...
    if (action === 'add') {
        text = chat.sWelcome || this.welcome || this.conn?.welcome || 'Benvenuto, @user!]';
    } else if (action === 'remove') {
        text = chat.sBye || this.bye || this.conn?.bye || 'Addio, @user!]';
    } else {
    ...
    for (let user of participants) {
        ...
        let finalText = text.replace('@user', '@' + user.split('@')[0])
