...
// Disabilitato il render immagini in terminale per evitare I/O pesante
const terminalImage = null

const urlRegex = /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/g

// Throttling console: max 10 log/sec, burst 5, e debounce per mittente
const LOG_WINDOW_MS = 100
const LOG_BURST_MAX = 5
let __logWindowStart = 0
let __logWindowCount = 0

function canLogNow() {
  const now = Date.now()
  if (now - __logWindowStart > LOG_WINDOW_MS) {
    __logWindowStart = now
    __logWindowCount = 0
  }
  if (__logWindowCount < LOG_BURST_MAX) {
    __logWindowCount++
    return true
  }
  return false
...
export default async function (m, conn = { user: {} }) {
  ...
  try {
    ...
    // Niente download media qui per non creare I/O
    ...
    if (isGroup && chatJid) {
      ...
      if (participantCount) righe.push(`${bordi.pipe} ${c.label('👥 Membri:')} ${c.text(participantCount)}`)
    }
    ...
    // Unica stampa aggregata
    console.log('\n' + righe.join('\n'))

    // Testo formattato leggero
...
async function safeGetName(conn, jid) {
  try { return await conn.getName(jid) || '' } catch { return '' }
}
...
async function formatText(m, conn) {
  ...
  // Evidenzia URL solo per testi piccoli (meno overhead)
  if (text.length < 2048) {
    text = text.replace(urlRegex, url => chalk.cyanBright(url))
  }

  // Menzioni con protezione error spam
  if (m.mentionedJid) {
