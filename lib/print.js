...
>>>>>>> e8d6ea9a7b40d0ef9c7d011b97b28c2816f5f578
    }
...
async function checkAdmin(conn, chatId, senderId) {
  try {
    ...
    const parts = Array.isArray(groupMeta?.participants) ? groupMeta.participants : []
    return parts.some(p => {
      ...
      try {
        ...
        if (m.isGroup && m.chat) {
          ...
          const participant = participants.find(p => {
            const pDecodedId = conn.decodeJid(p.id || p.jid || '');
            return pDecodedId === mentionJid;
          });
